...
namespace DickinsonBros.AccountAPI.Infrastructure.EmailSender
{
    ...
    {
        internal IGuidService _guidService;
        public EmailService(IGuidService guidService)
        {
        ...
        public async Task SendPasswordResetEmailAsync(string email, string passwordResetToken, System.Guid emailPreferenceToken)
        {
            var message = new MimeMessage();
            message.From.Add(new MailboxAddress("Roller Coaster Maker", "NoReply@RollerCoasterMaker.com"));
            message.To.Add(new MailboxAddress(email));
        ...
        {
            var path = Path.Combine("C:/Emails", System.Guid.NewGuid().ToString() + ".eml");
            Stream stream;
